  ...
  if (opt._ !== null) {
    ...
    if (command == 'list') {
      freckle.projects.list(function( err, projects ) {
        ...
        for (var i = projects.length - 1; i >= 0; i--) {
          project_names.push(projects[i].project.name + ":" + projects[i].project.id);
        }
        console.log(project_names.sort());
    ...
    if (command == 'log') {
      ...
      if (opt.project == null) {
        ...
        // Check to see if it's an integer if someone entered project id.
        var intRegex = /^\d+$/;
        if (!intRegex.test(opt.project)) {
          // @todo: Do a lookup to see if there is an exact project name.
          console.log('Invalid project: ' + opt.project + '. Please do a lookup (ginger list) and enter the project id.');
          data.missing.push('project');
        }
      }
